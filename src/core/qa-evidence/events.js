...
function refreshRequest(value) {
  exact(value, ['reason'], QA_EVENTS.REFRESH_REQUESTED);
  nonEmpty(value.reason, 'reason');
}
function exportRequest(value) {
  exact(value, ['format'], QA_EVENTS.EXPORT_REQUESTED);
  if (!['JSON','CSV'].includes(value.format)) throw new TypeError('qaEvidence:exportRequested format is invalid.');
}
function changed(value) {
  exact(value, ['reason','reviewModel','source'], QA_EVENTS.CHANGED);
  nonEmpty(value.reason, 'reason');
  if (!validateQaEvidenceSource(value.source).ok || !validateQaReviewModel(value.reviewModel).ok) throw new TypeError('qaEvidence:changed evidence is invalid.');
}
function failed(value) {
  exact(value, ['code','message'], QA_EVENTS.REFRESH_FAILED);
  nonEmpty(value.code, 'code'); nonEmpty(value.message, 'message');
}
function exact(value, keys, topic) {
