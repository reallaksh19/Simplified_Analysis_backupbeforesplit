...
export function createQaReviewModel(source) {
  ...
  const base = {
    ...
    qualityState,
    registrySummary: deepFreeze({
      ...source.registryReference,
      implementedCount: source.consumerRows.filter((row) => row.implementationStatus === IMPLEMENTATION_STATUS.IMPLEMENTED).length,
      unavailableCount: consumerSummary.unavailableCount,
    }),
