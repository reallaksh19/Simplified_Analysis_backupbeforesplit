...
function evidenceStates(key, contract, context, accepted, diagnostic) {
  ...
  return { validatorState:'VALID', datasetState:context?.datasetId && datasetId ? 'MATCHED' : 'NOT_APPLICABLE', linkState:hasApplicableLink(key) ? 'CURRENT' : 'NOT_APPLICABLE' };
}
...
function hasApplicableLink(key) { return !['sharedModel','loadCaseSet','modelCalculationLedger'].includes(key); }
