...
import { APPLICATION_NAVIGATION_ORDER_V9, CONTRACT_KEYS, IMPLEMENTATION_STATUS, READINESS_STATES, WORKSPACE_CONSUMER_CONTEXT_SCHEMA, WORKSPACE_CONSUMER_REGISTRY_V9_SCHEMA } from '../workspace-consumers/index.js';
import {
...
export function assertPlainJson(value, label = 'QA evidence') {
...
export function validateQaEvidenceExport(value) {
  ...
  ['mediaType','fileName','sourceSemanticHash','reviewSemanticHash','content'].forEach((field) => {
    if (typeof value?.[field] !== 'string' || !value[field]) errors.push(`QA export ${field} is invalid.`);
  });
...
export function qaSourceHashPayload(value) {
  ...
  return {
    schema: value?.schema, registryReference: value?.registryReference,
    contextReference: identityContext, consumerRows: value?.consumerRows,
    contractRows: value?.contractRows, diagnostics: value?.diagnostics,
...
export function qaReviewHashPayload(value) {
  ...
  const { workspaceVersion: _workspaceVersion, ...identityContext } = context;
  return {
    schema: value?.schema, qualityState: value?.qualityState,
    registrySummary: value?.registrySummary, contextSummary: identityContext,
    consumerRows: value?.consumerRows, contractRows: value?.contractRows,
    diagnostics: value?.diagnostics, limitations: value?.limitations,
  };
}

export function canonicalDiagnostics(rows = []) {
  const copied = JSON.parse(canonicalStringify(rows));
  return deepFreeze(copied.sort((a, b) => diagnosticKey(a).localeCompare(diagnosticKey(b))));
}
...
function validateContextCounts(context, rows, errors) {
  ...
  const invalid = rows.filter((row) => row.availability === 'INVALID').length;
  const unavailable = rows.filter((row) => row.availability === 'UNAVAILABLE').length;
...
function validateReviewCounts(summary, rows, errors) {
  ...
  const unavailable = implemented.filter((row) => row.readinessState !== READINESS_STATES.AVAILABLE).length;
...
function validateEvidenceRows(value, errors) {
  ...
  validateExactMembership(value?.consumerRows, 'consumerId', APPLICATION_NAVIGATION_ORDER_V9, 'consumer rows', errors);
...
function validateSourceReference(registry, context, errors) {
  ...
  validateRegistryReferenceValues(registry, errors);
  validateContextReferenceValues(context, errors);
}
...
function validateRegistryReferenceValues(registry, errors) {
  if (registry?.schema !== WORKSPACE_CONSUMER_REGISTRY_V9_SCHEMA) errors.push('QA registry reference schema is invalid.');
  if (typeof registry?.semanticHash !== 'string' || !registry.semanticHash) errors.push('QA registry reference semantic hash is invalid.');
  if (registry?.consumerCount !== APPLICATION_NAVIGATION_ORDER_V9.length) errors.push('QA registry reference consumer count is invalid.');
  if (canonicalStringify(registry?.consumerIds) !== canonicalStringify(APPLICATION_NAVIGATION_ORDER_V9)) errors.push('QA registry reference consumer order is invalid.');
...
function validateContextReferenceValues(context, errors) {
  ...
  const counts = ['availableContractCount','invalidContractCount','unavailableContractCount','diagnosticCount'];
  counts.forEach((field) => { if (!Number.isInteger(context?.[field]) || context[field] < 0) errors.push(`QA context ${field} is invalid.`); });
  const total = (context?.availableContractCount || 0) + (context?.invalidContractCount || 0) + (context?.unavailableContractCount || 0);
  if (total !== CONTRACT_KEYS.length) errors.push('QA context contract counts are inconsistent.');
  if (context?.validationState === 'VALID_EMPTY') {
    if (context.schema !== null || context.contextId !== null || context.semanticHash !== null || context.datasetId !== null) errors.push('QA empty context identity is invalid.');
  } else if (context?.validationState === 'VALID') {
    if (context.schema !== WORKSPACE_CONSUMER_CONTEXT_SCHEMA || typeof context.contextId !== 'string' || !context.contextId || typeof context.semanticHash !== 'string' || !context.semanticHash) errors.push('QA context identity is invalid.');
...
function validateConsumerRow(row, errors) {
  ...
  ['availableRequiredCount','availableOptionalCount'].forEach((field) => {
    if (!Number.isInteger(row?.[field]) || row[field] < 0) errors.push(`QA consumer ${row?.consumerId || ''} ${field} is invalid.`);
  });
}
function validateContractRow(row, errors) {
  ...
  validateCanonicalStrings(row?.requiredByConsumerIds, `${row?.contractKey || ''} required consumers`, errors);
  validateCanonicalStrings(row?.optionalForConsumerIds, `${row?.contractKey || ''} optional consumers`, errors);
  validateCanonicalArray(row?.diagnostics, diagnosticKey, `${row?.contractKey || ''} diagnostics`, errors);
  if (!Array.isArray(row?.qualificationSummary)) errors.push(`QA contract ${row?.contractKey || ''} qualification summary is invalid.`);
  validateContractStates(row, errors);
}
function validateContractStates(row, errors) {
  if (!QA_AVAILABILITY_STATES.includes(row?.availability)) errors.push(`Contract ${row?.contractKey || ''} availability is invalid.`);
  if (!QA_VALIDATOR_STATES.includes(row?.validatorState)) errors.push(`Contract ${row?.contractKey || ''} validator state is invalid.`);
  if (!QA_DATASET_STATES.includes(row?.datasetState)) errors.push(`Contract ${row?.contractKey || ''} dataset state is invalid.`);
  if (!QA_LINK_STATES.includes(row?.linkState)) errors.push(`Contract ${row?.contractKey || ''} link state is invalid.`);
}
function validateExactMembership(rows, key, expected, label, errors) {
  if (!Array.isArray(rows)) return;
  if (canonicalStringify(rows.map((row) => row?.[key])) !== canonicalStringify(expected)) errors.push(`QA ${label} membership or ordering is not canonical.`);
}
function validateRows(rows, key, label, errors) {
  if (!Array.isArray(rows)) return errors.push(`QA ${label} must be an array.`);
  const keys = rows.map((row) => row?.[key]);
  if (keys.some((value) => typeof value !== 'string' || !value)) errors.push(`QA ${label} contain an invalid ${key}.`);
  if (new Set(keys).size !== keys.length) errors.push(`QA ${label} contain duplicate ${key} values.`);
}
function validateCanonicalStrings(value, label, errors) {
  if (!Array.isArray(value) || value.some((row) => typeof row !== 'string')) return errors.push(`QA ${label} are invalid.`);
  if (canonicalStringify(value) !== canonicalStringify([...new Set(value)].sort())) errors.push(`QA ${label} are not canonical.`);
}
function validateCanonicalArray(value, selector, label, errors) {
  if (!Array.isArray(value)) return errors.push(`QA ${label} must be an array.`);
  const keys = value.map(selector);
  if (new Set(keys).size !== keys.length || canonicalStringify(keys) !== canonicalStringify([...keys].sort())) errors.push(`QA ${label} are duplicated or non-canonical.`);
}
function exactKeys(value, keys, label, errors) {
  if (!isPlain(value)) return errors.push(`${label} must be a plain record.`);
  if (canonicalStringify(Object.keys(value).sort()) !== canonicalStringify([...keys].sort())) errors.push(`${label} fields are not exact.`);
}
function hashCheck(value, payload, label, errors) {
  if (value?.semanticHash !== semanticHash(payload)) errors.push(`${label} semantic hash mismatch.`);
}
function validatePublicEvidence(value, label, errors) {
  try { assertPlainJson(value, label); } catch (error) { errors.push(error.message); }
  if (!isDeepFrozen(value)) errors.push(`${label} public evidence must be deeply frozen.`);
}
function walk(value, path, seen) {
  ...
  if (Array.isArray(value)) {
    if (Object.keys(value).length !== value.length) throw new TypeError(`${path} contains a sparse or extended array.`);
    value.forEach((item, index) => walk(item, `${path}[${index}]`, seen));
  } else {
    if (!isPlain(value) || Object.getOwnPropertySymbols(value).length) throw new TypeError(`${path} contains a non-plain record.`);
    Object.keys(value).forEach((key) => walk(value[key], `${path}.${key}`, seen));
  }
  seen.delete(value);
}
