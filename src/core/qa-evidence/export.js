...
function jsonContent(source, reviewModel) {
  return `${canonicalStringify({ source, reviewModel })}\n`;
}
...
function append(target, group, columns, rows) {
  target.push([group, ...columns].map(csvCell).join(','));
  rows.forEach((row) => target.push([group, ...columns.map((column) => scalar(row[column]))].map(csvCell).join(',')));
}
function scalar(value) {
  if (Array.isArray(value) || (value && typeof value === 'object')) return canonicalStringify(value);
  return value ?? '';
}
