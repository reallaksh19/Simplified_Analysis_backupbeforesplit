...
test('blocking import loss fails closed and preserves the active dataset and draft', async ({ page }) => {
  ...
  const navigation = page.getByRole('navigation', { name: 'Application views' });
  await expect(navigation.getByRole('button', { name: 'QA',exact: true })).toHaveAttribute('aria-disabled', 'false');
  await expect(navigation.getByRole('button', { name: 'Local FEA',exact: true })).toHaveAttribute('aria-disabled', 'false');
