...
function checkContracts(){const constants=read('src/core/workspace-consumers/constants.js'),registry=read('src/core/workspace-consumers/registry.js'),views=read('src/core/workspace-consumers/view-state.js'),contracts=`${constants}\n${registry}\n${views}`;['workspace-consumer-registry/v1','workspace-consumer-registry/v2','application-view-state/v1','application-view-state/v2'].forEach((token)=>{if(!contracts.includes(token))errors.push(`Versioned contract token ${token} is missing.`);});['createWorkspaceConsumerRegistry','createWorkspaceConsumerRegistryV2','createApplicationViewStateV2','validateApplicationViewStateV2'].forEach((token)=>{if(!`${registry}\n${views}`.includes(token))errors.push(`Version compatibility proof ${token} is missing.`);});}
